...
<body
    ...
    <div style="width:100%; padding:24px 0;">
        <div
            ...
            <div
                style="background:linear-gradient(90deg,#10b981 0%,#059669 100%); padding:24px 20px; text-align:center;">
                <h2 style="color:#fff; margin:0; font-size:20px;">Task Updated in "{{ $project->name ?? 'Project' }}"
                </h2>
                <p style="color:rgba(255,255,255,0.9); margin:8px 0 0; font-size:13px;">A task has been updated — check
                    out the changes.</p>
            </div>
            ...
            <div style="padding:20px 24px; color:#0f172a;">
                ...
                <p style="margin:0 0 12px 0; color:#334155; font-size:14px; line-height:1.6;">
                    A task in the project <strong>{{ $project->name ?? '—' }}</strong> has been updated by
                    <strong>{{ $task->creator->name ?? ($project->owner->name ?? 'Project Owner') }}</strong>.
                </p>

                <div
                    style="background:#f8fafc; border-left:4px solid #10b981; padding:14px; border-radius:6px; margin:12px 0;">
                    <p style="margin:0 0 8px 0; font-weight:600;">Updated Task:</p>
                ...
                <div style="text-align:center; margin:18px 0;">
                    <a href="{{ config('app.url') . '/projects/' . ($project->id ?? '') }}"
                        style="display:inline-block; background:#10b981; color:#fff; padding:10px 18px; text-decoration:none; border-radius:6px; font-weight:600;">View
                        Project</a>
                </div>
